...
def train_model(config: dict):
    ...
    for epoch in range(initial_epoch, config['num_epochs']):
        ...
        # delete files generated by previous epoches
        if epoch > 0:
            previous_model_filename = get_weights_file_path(config, f"{epoch - 1:02d}")
            if Path(previous_model_filename).exists():
                Path(previous_model_filename).unlink()
